...
int main() {
    ...
    auto window = glfwCreateWindow(WIDTH, HEIGHT, "Lighting - Colors", nullptr, nullptr);
    if (!window) {
    ...
    lighting_shader.attachFromFile("02-lighting/08-colors-lighting.vert")
                   .attachFromFile("02-lighting/08-colors-lighting.frag")
                   .link();
    Shader lamp_shader;
    lamp_shader.attachFromFile("02-lighting/08-colors-lamp.vert")
               .attachFromFile("02-lighting/08-colors-lamp.frag")
               .link();
    ...
    container_vao.bind();
    vbo.vertexAttribPointer(pos_attrib, 3, GL_FLOAT, GL_FALSE, 3*sizeof(GL_FLOAT), (GLvoid*)0);
